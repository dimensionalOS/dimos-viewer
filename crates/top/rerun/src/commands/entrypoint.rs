...
fn run_impl_with_wrapper(
    ...
    if args.test_receive || args.save.is_some() {
        ...
        cfg_select! {
            feature = "native_viewer" => start_native_viewer_with_wrapper(
                &args,
                url_or_paths,
                _main_thread_token,
                _build_info,
...
/// `startup_patch` applied to [`re_viewer::StartupOptions`] and the
/// `app_wrapper` applied to the finished [`re_viewer::App`].
#[cfg(feature = "native_viewer")]
...
fn start_native_viewer_with_wrapper(
    ...
    if args.headless {
        if app_wrapper.is_some() {
            re_log::warn!(
                "Ignoring the dimos-viewer app wrapper: it is not supported in --headless mode."
            );
        }
        ...
        re_viewer::run_native_app(
            ...
            Box::new(move |cc| {
                ...
                // Apply the DimOS wrapper if provided, otherwise return the stock App.
                match app_wrapper {
                    Some(wrapper) => wrapper(app),
                    None => Ok(Box::new(app)),
                }
            }),
            renderer,
